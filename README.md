...
This is a fork of [TBXark/mail2telegram](https://github.com/TBXark/mail2telegram), which adds a Telegram Mini Apps email list, supports more operations in Mini Apps, better HTML preview, and supports the Ollama API.
...
| OLLAMA_API_ENDPOINT | Ollama API endpoint URL (e.g., `http://localhost:11434/api/chat`). When set, enables email summarization using a local Ollama instance. Provider priority: Workers AI > OpenAI > Ollama. |
| OLLAMA_MODEL | Ollama model name to use for summarization. Default value is `llama3.2`. |
| OLLAMA_API_KEY | Optional API key for Ollama instances that require authentication. Leave empty for local instances without authentication. |
| AUTO_SUMMARY | Set to `true` to automatically generate AI summary when a new email arrives. The summary will be cached for faster response when clicking the "AI Summary" button. Default is disabled. |
...
2. `Summary` mode: When Workers AI is bound with `WORKERS_AI_MODEL`, summaries are generated by Workers AI. If not, but `OPENAI_API_KEY` is set, OpenAI is used instead. If neither is available but `OLLAMA_API_ENDPOINT` is set, Ollama is used for local summarization. Without any of these options, the `Summary` button is hidden.
